...
\begin{definition}\label{def:dualscaledsys:iqc}
    For the dual realization $G^\dual$ as in Definition~\ref{def:dualrealization}, and the dual
    filter $\mbf{D}(\Psi)\in\mcl{L}(\RL)$, the dynamically filtered dual system is expressed
    in terms of $\tilde{\underline{w}}(t)\in\RL^{\mbf{n}_z}$ and
    $\tilde{\underline{z}}(t)\in\RL^{\mbf{n}_w}$, where the unfiltered dual variables satisfy
    $\underline{\mbf{x}}(0)=0$ and $\underline{z}=G^\dual\underline{w}$.
    The dual scaled variables are defined, analogously to the primal scaled variables, by
    ...
    Equivalently, since $\mbf{D}(\Psi)^{-1}=\hat{\mcl{J}}^*\Psi^\dual\hat{\mcl{J}}$, the inverse
    filter relation has the state-space representation
    \begin{equation}\label{eqn:ssdualscaling:iqc}
        ...
        \bmat{\mcl{A}_\Psi^* & -\mcl{C}_\Psi^{w*} & \mcl{C}_\Psi^{z*} \\
            ...
            \bmat{\underline{\mbf{x}}_\Psi(t) \\ \tilde{\underline{z}}(t) \\ \tilde{\underline{w}}(t)}
    \end{equation}
\end{definition}
...
\begin{example}[Dual multiplier for a zero-padded extension of the LTI-parametric IQC]
    ...
    \begin{equation}
        ...
        \mcl{Q}-\mcl{S}\mcl{Q}^{-1}\mcl{S}.
    \end{equation}
    Then $\Theta\succ0$, and the multiplication operator $\mcl{M}$ has
    the lower triangular $J$-factorization induced by
    $\mcl{P}=\mcl{V}^*J\mcl{V}$, where
    \begin{equation}
        \mcl{V}
        :=
        \bmat{
            \Theta^{1/2} & 0\\
            \mcl{Q}^{-1/2}\mcl{S} & \mcl{Q}^{1/2}
        },
        \qquad
        J:=
        \bmat{I & 0\\0 & -I},
    ...
    \begin{equation}
        ...
        =
        \bmat{
            \Theta^{1/2}\psi_\square
            &
            0\\
            \mcl{Q}^{-1/2}\mcl{S}\psi_\square
            &
            \mcl{Q}^{1/2}\psi_\square
        }.
    \end{equation}
    ...
    \end{equation}
    Hence the pointwise multiplication operator
    $\mcl{M}_d\in\mcl{L}(\mbf L_{[a,b]})$ is induced by
    \begin{equation}
        \mcl{P}_d:=\mcl{V}_d^*J\mcl{V}_d,
        \qquad
        (\mcl{M}_d v)(t):=\mcl{P}_d v(t).
    \end{equation}
    Thus the squared-up dual inequality is
    \begin{equation}
        \ip{
            p_T\Psi_{d,\square}
    ...
    \begin{equation}
        ...
        \bmat{\mcl{Q} & \mcl{S}^*\\
              \mcl{S} & -\mcl{Q}} .
    \end{equation}
    With the same $\Theta$, the block inverse is
    \begin{equation}
        \mcl{P}_d
        =
        \bmat{
    ...
    \begin{equation}
        ...
        .
    \end{equation}
    The induced dual inequality for this zero-padded extension is
    therefore, for all $T\geq0$ and all $\underline{z}(t),\underline{w}(t)\in\RL$
    satisfying $\underline{z}=G^\dual\underline{w}$,
    \begin{equation}
        \ip{
            p_T
            \Psi_d
