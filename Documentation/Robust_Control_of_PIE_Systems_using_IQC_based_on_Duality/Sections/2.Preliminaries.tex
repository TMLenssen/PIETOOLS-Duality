...
\] The space $L_2^n[0,\infty)$ denotes $\R^n$-valued square-integrable signals defined on the temporal domain $[0,\infty)$, so that $x(t) \in \R^n$. Similarly, $\RL^n[0,\infty)$ denotes $\RL$-valued square-integrable temporal signals. For suitably differentiable $\mathbf{x} \in \RL^n[0,\infty)$, we write $\mathbf{\dot{x}} := \frac{\partial \mathbf{x}}{\partial t}$. For a Hilbert space $\mathbf{H}$, we denote by $\mathbf{L}_{\mathbf{H}}$ the space of square-integrable functions $u : [0,\infty) \to \mathbf{H}$ with inner product
\[
...
\begin{definition}[PIE System]\label{def:PIE}
	Given the PIE system $G\{\mcl{T}, \mcl{A}, \mcl{B}, \mcl{C}, \mcl{D}\}$
	with PI operators $\mcl{T}, \mcl{A}, \mcl{B}, \mcl{C},$ and $\mcl{D}$ of compatible dimensions,
	and signals $w(t)\in \RL^{\mbf{n}_w}[a,b]$, we say $\mbf{x}(t)\in\RL[a,b]^{\mbf{n}_x}$ and $z(t)\in \RL^{\mbf{n}_z}[a,b]$ 
	...
	\end{equation}
	for all $t\geq0$ and $\mcl{T}\mbf{x}(0) = \mcl{T}\mbf{x}_0$. Additionally, for signals $\underline{w}(t)\in\RL[a,b]^{\mbf{n}_z}$
	of compatible dimensions, we say $\underline{\mbf{x}}(t)\in\RL[a,b]^{\mbf{n}_x}$ and $\underline{z}(t)\in L_2[a,b]^{\mbf{n}_w}$
    ...
    \end{equation}
	for all $t\geq0$ and $\mcl{T}^*\underline{\mbf{x}}(0) = \mcl{T}^*\underline{\mbf{x}}_0$.
\end{definition}

\begin{definition}[Generalization of $\R\mathbb{H}_\infty$]\label{def:RHinf}
	Let $G$ be a PIE system as in Definition~\ref{def:PIE}. We say $G$ is internally stable and bounded with decay rate
	$\alpha>0$, $M>0$, and $L_2$-gain $\gamma>0$ if
	\begin{itemize}
		\item The homogeneous part \(\partial_t(\mcl{T}\mbf{x})(t)=\mcl{A}\mbf{x}(t)\), with \(\mcl{T}\mbf{x}(0)=\mcl{T}\mbf{x}_0\)
		...
		\item With zero initial condition $\mbf{x}(0)=0$, then for
		      ...
		      \]
	\end{itemize}
	Clearly, $\R\mathbb{H}_\infty\subset\R\mathbb{H}_\infty^{\mathrm{PIE}}$ \cite{TsaiGu2014RobustOptimalControl}. 
	Also $G^\top$ is in $\R\mathbb{H}_\infty^{\mathrm{PIE}}$ with the same decay rate, $L_2$-gain, and $M$ \cite{shivaknew}.
\end{definition}
