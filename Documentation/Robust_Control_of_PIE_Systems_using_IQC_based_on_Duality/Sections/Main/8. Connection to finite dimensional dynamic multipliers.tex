...
define $\Pi = \Psi^\sim V\Psi\in\R L_\infty$, where $\Psi(j\omega)^\sim = \Psi(-j\omega)^\top$.
\begin{theorem}\label{thm:strict-pn-j-spectral}
Let $\Pi=\Pi^\sim\in\R L_\infty^{(n_{\mrm{z}}+n_{\mrm{w}})\times(n_{\mrm{z}}+n_{\mrm{w}})}$
where $\Pi_{11}\in\R L_\infty^{n_{\mrm{z}}\times n_{\mrm{z}}}$ and
$\Pi_{22}\in\R L_\infty^{n_{\mrm{w}}\times n_{\mrm{w}}}$. If $\Pi_{11}(j\omega)\succ0$,\,$\Pi_{22}(j\omega)\prec0$,
for all $\omega\in\mathbb{R}\cup\{\infty\}$,
then
\begin{enumerate}
    \item $\Pi$ admits a $J_{n_{\mrm{z}},n_{\mrm{w}}}$-spectral factorization
          $\Pi=\Theta^\sim J_{n_{\mrm{z}},n_{\mrm{w}}}\Theta$, where
          $\Theta,\Theta^{-1}\in
          \R H_\infty^{(n_{\mrm{z}}+n_{\mrm{w}})\times(n_{\mrm{z}}+n_{\mrm{w}})}$.
    \item $(\Theta,J_{n_{\mrm{z}},n_{\mrm{w}}})$ is a hard factorization of $\Pi$.
\end{enumerate}
\end{theorem}
Pfifer and Seiler~\cite[App.~B]{Pfifer2015} give a Riccati-based construction
of $J$-spectral factors for strict PN multipliers, using the rational
$J$-spectral factorization results of Meinsma~\cite{MEINSMA1995243}.
...
\subsection{Example. Generation of \texorpdfstring{$\beta$}{beta}-Oscillations in the Subthalamic Nucleus-Globus Pallidus Network}
Consider the delayed firing-rate model of Nevado Holgado, Terry, and
Bogacz~\cite[Eq.~(5)]{nevadoHolgado2010betaOscillations}, and define the
stimulation input $\tilde u(t):=I_{\mrm{e}}\mrm{Stim}(t)$.
\begin{equation}
      \begin{aligned}
...
respectively. To formulate the control objective around a desired operating point, we introduce the equilibrium
 \((\tilde x_{\mrm{S}}^\star,\tilde x_{\mrm{G}}^\star)\) satisfying
\begin{align*}
...
origin.
Following the delay representation of
Peet~\cite{peet2020representationnetworkssystemsdelay}, we introduce 
...
\settowidth{\philhswidth}{$\dot{\boldsymbol{\phi}}_{ij}(t,s)\mathrel{}$}
...
For $D:=\operatorname{diag}(d_{\mrm{S}},d_{\mrm{G}})\succ0$, the sector
...
feasible bound.
For the present parameterization, the largest certified sector bound is
$\beta_{\mrm{sec}}^*=0.492$.

\subsubsection{Zames--Falb multiplier}
Zames and Falb~\cite{zames1968stability} introduced multipliers that exploit
monotonicity and slope restrictions on nonlinearities. To exploit
\eqref{eq:stn-gpe-slope-bound}, we replace the static weight $D$ by a diagonal
Zames--Falb multiplier using the construction of
Turner~\cite{turner2022zamesfalbmultipliersdontpanic},
...
impossible.
For the present parameterization, the largest certified Zames--Falb bound is
$\beta_{\mrm{ZF}}^*=0.562$.
...
\qquad
\Theta_\Delta,\Theta_\Delta^{-1}\in\R H_\infty.
\end{equation*}
...
$\gamma_{\mrm{S}}$, $\gamma_{\mrm{G}}$, and $\gamma_{\mrm{u}}$ for
...
\|u\|_{L_2}&\leq\gamma_{\mrm{u}}\|w_{\mrm{p}}\|_{L_2}.
\end{aligned}
\end{equation*}
...
We will report all three bounds and compare $\gamma_{\mrm{u}}$ with
