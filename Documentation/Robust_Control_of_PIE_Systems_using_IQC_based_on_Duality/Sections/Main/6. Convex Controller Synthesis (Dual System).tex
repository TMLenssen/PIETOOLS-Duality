...
\begin{corollary}\label{thm:state-feedback}
    ...
    Eq.~\eqref{eq:primal-system-components}, and let
    $P_K:=K\star G$ denote the generalized plant obtained after closing the
    controller loop.
    ...
    \begin{itemize}
        \item The interconnection $P_K\star\Delta$ is well-posed for all $\Delta\in\mbf{\Delta}$.
...
\]
Then the interconnection $P_K\star\Delta$ with $\mbf{x}(0)=0$ is robustly stable and robust performance is achieved.
\end{corollary}
\begin{proof}
