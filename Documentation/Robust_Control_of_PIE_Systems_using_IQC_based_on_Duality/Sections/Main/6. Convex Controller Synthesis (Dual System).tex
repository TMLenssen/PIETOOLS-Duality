...
include robust estimation~\cite{scherer2008robustness} and disturbance-feedforward
controller synthesis~\cite{kose2009robust,venkataraman}.
...
If $\Theta\neq I$, assume that $z,w$ are measurable.
...
\begin{corollary}\label{thm:state-feedback}
    ...
    Let $\Delta\in\mbf{\Delta}$ and suppose a primal hard IQC has the hard
    $(\Theta,J_{\mbf n_z,\mbf n_w})$ factorization. Assume that
    \begin{itemize}
...
\[
\bmat{\mcl K_P&\mcl K_\Theta}=\mcl Z^*\mcl P^{-1}.
\]
Assume that $\mcl D_{21}\mcl D_P+\mcl D_{22}$ is invertible, with
\[
\Lambda:=(\mcl D_{21}\mcl D_P+\mcl D_{22})^{-1}\in\Pi_4
\]
