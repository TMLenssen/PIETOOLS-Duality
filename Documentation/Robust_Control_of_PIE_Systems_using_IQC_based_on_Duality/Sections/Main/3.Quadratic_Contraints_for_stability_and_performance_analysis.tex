...
then $p_\tau(z_\Delta - \hat{z}_\Delta) = 0$ and
$p_\tau(w_\Delta - \hat{w}_\Delta) = 0$, for all
$\tau \geq 0$.
\end{itemize}
\end{definition}
...
$\Psi \in
\mbf L^{\mbf{n}_{w_\Delta} + \mbf{n}_{z_\Delta}}_{e, [a, b]}
\to
L^{\mbf{n}_{w_\Delta} + \mbf{n}_{z_\Delta}}_{e, [a, b]}$
multiplier $\mcl M_{\mcl V}\in \mcl L(\mbf L_{[a, b]})$,
parameterized by
...
\ip{p_T \Psi \bmat{z_\Delta \\ w_\Delta}}
{p_T \mcl{M}_{\mcl{V}_\Delta}\Psi\bmat{z_\Delta \\ w_\Delta}}
_{\mbf L_{[a, b]}}
\geq 0.
\end{equation}
Moreover, let
...
simultaneous robust stability and performance verification.

\begin{theorem}[Robust Stability and Performance]\label{thm:robust-stability-performance}
Let $P$, $\Delta\in\mbf{\Delta}$ be given, and there exists $\Psi$,
...
\begin{gathered}
\ip{p_T\hat{P}\bmat{w_\Delta\\w_p}}
{p_T\bmat{
\mcl{M}_{\mcl{V}_\Delta} & 0 \\
0 & \mcl{M}_{{\mcl V}_p}
}\hat{P}\bmat{w_\Delta\\w_p}}_{\mbf L_{[a, b]}} \\
\leq
-\epsilon
(\|p_T{w_\Delta}\|^2_{\mbf L_{[a, b]}}+\|p_T w_p\|^2_{\mbf{L}_{[a,b]}})
\end{gathered}
\end{equation}
for all $w_\Delta \in \mbf L_{e,[a,b]}^{\mbf{n}_{w_\Delta}}$,
$w_p\in \mbf L_{e,[a,b]}^{\mbf{n}_{w}}$ and $T \geq 0$, where
$\hat{P} :=
\bmat{\Psi_\Delta & 0 \\ 0&  I}
\left[
\begin{array}{cc}
P_\Delta & P_{\Delta p} \\
I & 0 \\ \hdashline
...
\end{array}
\right]$
according to \eqref{eq:uncertain-interconnection-map}.
\end{theorem}
\begin{proof}
