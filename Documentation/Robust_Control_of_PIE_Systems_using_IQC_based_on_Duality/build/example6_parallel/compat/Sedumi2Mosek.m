...
%     cell implementation
    ...
    % 2026-09-10: Sparse vectorization of the original row-wise conversion.
    % Fold upper/lower entries onto the lower triangle, averaging off-diagonals.
    [row,entry,value]=find(A(:,Kindex(j):Kindex(j+1)-1));
    nk=K.s(j);
    kr=mod(entry-1,nk)+1;
    kc=floor((entry-1)/nk)+1;
    value(kr~=kc)=value(kr~=kc)/2;
    lower=max(kr,kc); upper=min(kr,kc);
    folded=sparse(row,lower+nk*(upper-1),value,m,nk^2);
    [row,entry,value]=find(folded);
    [~,order]=sortrows([row,entry],[1,2]);
    row=row(order); entry=entry(order); value=value(order);
    prob2.bara.subi=[prob2.bara.subi,row'];
    prob2.bara.subj=[prob2.bara.subj,j*ones(1,numel(row))];
    prob2.bara.subk=[prob2.bara.subk,(mod(entry-1,nk)+1)'];
    prob2.bara.subl=[prob2.bara.subl,(floor((entry-1)/nk)+1)'];
    prob2.bara.val=[prob2.bara.val,value'];
end
