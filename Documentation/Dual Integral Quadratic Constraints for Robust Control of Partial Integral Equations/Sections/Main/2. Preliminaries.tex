...
write $L_2^n$ or $\R L_2^{\mathbf{n}}$. We omit dimensions when clear from context.
...
\mcl{T}^*\underline{\mbf{x}}_0$. A notational convenction
is using underlining anything related to the dual system.
For these systems, we consider input--output and exponential stability. 
Given a PIE system $G$, we say $G$ is input-output stable if for any $z, w \in L_{\mrm{e},[a,b]}$ satisfying $z = G w$ with $\mbf{x}_0=0$, consistently with~(\ref{ass:initial-conditions}), we have
\[
\|p_T z\|_{L_{[a,b]}}
\leq
...
We say that the interconnection $G\star H$ is
\emph{well--posed} if
\begin{itemize}
\item \emph{Existence and Uniqueness}, For any
$u_2\in L_{\mrm{e}, [a,b]}$, there exist unique
$y_2\in L_{\mrm{e}, [a,b]}$,
$y_1 \in L_{\mrm{e}, [a,b]}$ and
$u_1 \in  L_{\mrm{e}, [a,b]}$ such that $y_2$, $u_2$,
$y_1$, $u_1$ satisfy the interconnection.
\item \emph{Causality}, If $y_1$, $u_1$ and
$\hat{y}_1$, $\hat{u}_1$ satisfy the interconnection,
then $p_T(y_1 - \hat{y}_1) = 0$ and
$p_T(u_1 - \hat{u}_1) = 0$, for all
$T \geq 0$.
\end{itemize}
\subsection{PIETOOLS}
