...
while ~exhausted || any(~cellfun(@isempty,values(active)))
    for slot=1:numWorkers
        if isempty(active(slot)) && ~exhausted
            job=nextJob();
            if isempty(job)
                exhausted=true;
                break
    ...
    for slot=1:numWorkers
        if isempty(active(slot)) || ~active(slot).process.HasExited, continue; end
        worker=active(slot);
        outputFile=fullfile(worker.directory,'output.mat');
        try
    ...
    if any(~cellfun(@isempty,values(active))), pause(.25); end
end
