function plot_Example_6(result)
% Show only accepted PI certificates; leave solver failures as gaps.
here=fileparts(mfilename('fullpath'));
if nargin==0
    data=load(fullfile(here,'Example_6_Fig7_dual.mat')); result=data.Results;
end
...
sgtitle(['Coupled heat equations: ' result.side ' IQC gain bounds']);
exportgraphics(f,fullfile(here,['fig7_heat_' result.side '.png']),'Resolution',180);
