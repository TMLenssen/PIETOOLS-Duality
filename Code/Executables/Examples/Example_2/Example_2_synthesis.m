...
%% Example 4 with boundary control
%
%   x_t = x_ss + lambda*sin(x),
...
% The controller is u=K*[xb;x].
pvar t s
a = 0;
b = 1;
alpha = -1;
beta = 1;
kypSlackMode = 'normal';     % 'normal' or 'signed'
residualFactor = 1.1;
...
while lambdaUpper-lambdaLower > lambdaTolerance
    ...
    [feasible,Ktrial,eps,residual,feasratio,numerr] = ...
        synthesize_lambda(lambdaTrial,a,b,alpha,beta,settings,t,s, ...
        residualFactor);
...
%% Simulate phi(x)=sin(x) at the final certified lambda
if runSimulation
    [simOpen,simClosed] = simulate_best_controller( ...
        lambdaLower,K,a,b,t,s,simulationFinalTime);
    fprintf('\nSimulation at lambda = %.6g with phi(x)=sin(x)\n',lambdaLower);
    ...
    if plotSimulation
        plot_simulation(simOpen,simClosed);
    end
end

function [feasible,K,eps,residual,feasratio,numerr] = ...
        synthesize_lambda(lambda,a,b,alpha,beta,settings,t,s,residualFactor)
P = boundary_control_plant(lambda,a,b,t,s);
...
function P = boundary_control_plant(lambda,a,b,t,s)
...
function P = boundary_control_simulation_plant(lambda,a,b,t,s)
...
P = boundary_control_simulation_plant(lambda,a,b,t,s);
nonlinearity = @(x) sin(x);
tgrid = linspace(0,tFinal,501).';
splot = linspace(a,b,151).';

x0.ode = 0;
x0.pde = {@(position) 0.05*sin(0.5*pi*position)};
simOpts.N = 20;
simOpts.splot = splot;
simOpts.statePIE = P;
simOpts.nwd0 = 0;
...
function fig = plot_simulation(simOpen,simClosed)
fig = figure('Color','w');
tiledlayout(1,3,'TileSpacing','compact','Padding','compact');
nexttile;
surf(simOpen.splot,simOpen.t,simOpen.zPlot,'EdgeColor','none');
xlabel('s'); ylabel('t'); zlabel('x(s,t)'); title('Open loop');
view(3); axis tight;
nexttile;
surf(simClosed.splot,simClosed.t,simClosed.zPlot,'EdgeColor','none');
xlabel('s'); ylabel('t'); zlabel('x(s,t)'); title('Boundary controlled');
view(3); axis tight;
nexttile;
