...
num_r = numel(r_grid);
num_a = numel(alpha_grid);
gamma_grid = nan(num_a, num_r);
feasible_grid = false(num_a, num_r);
evaluated_grid = false(num_a, num_r);
feasratio_grid = nan(num_a, num_r);
pinf_grid = false(num_a, num_r);
dinf_grid = false(num_a, num_r);
message_grid = strings(num_a, num_r);
K_grid = cell(num_a, num_r);
Psol_grid = cell(num_a, num_r);
Zsol_grid = cell(num_a, num_r);

for i_a = 1:num_a
    a_val = alpha_grid(i_a);
    for i_r = 1:num_r
        r = r_grid(i_r);
        try
            ...
            PDE = [diff(x1,t) ==x2;
                ...
                z  == [x3;x4;0.05*u];
                zd == [(a_val)*(kt/m2)*(subs(ph2,s,b) - subs(ph1,s,b)); (a_val)*(ct/m2)*(subs(ph4,s,b) - subs(ph3,s,b))];
                subs(ph1,s,a) == x1;
                subs(ph2,s,a) == x3;
                subs(ph3,s,a) == x2;
                subs(ph4,s,a) == x4];
            ...
            fprintf('\n- Solving robust synthesis LPI for alpha = %g, r = %d...\n', a_val, r);

            prog = lpiprogram(PIE.vars(:,1), PIE.vars(:,2), PIE.dom);
            evaluated_grid(i_a, i_r) = true;
            ...
            [is_feasible, pinf_grid(i_a, i_r), dinf_grid(i_a, i_r), ...
                feasratio_grid(i_a, i_r)] = solved_feasible(prog);

            if is_feasible
                gam_val = double(lpigetsol(prog, gam_dec));
                gamma_grid(i_a, i_r) = gam_val;
                feasible_grid(i_a, i_r) = true;

                Psol = lpigetsol(prog, Pdec);
                Zsol = lpigetsol(prog, Zdec);
                Psol_grid{i_a, i_r} = Psol;
                Zsol_grid{i_a, i_r} = Zsol;
                K_grid{i_a, i_r} = recover_controller_gain(PIE, Psol, Zsol, r, nzd, nwd, tol);

                fprintf('  feasible, gamma = %.6g\n', gam_val);
            else
                message_grid(i_a, i_r) = "Solver reported infeasible or unreliable status.";
                fprintf('  not accepted: pinf = %d, dinf = %d, feasratio = %.4g\n', ...
                    pinf_grid(i_a, i_r), dinf_grid(i_a, i_r), feasratio_grid(i_a, i_r));
            end
        catch ME
            message_grid(i_a, i_r) = string(ME.message);
            fprintf('  failed: %s\n', ME.message);
        end
    end
end
...
[best_alpha_idx, best_r_idx] = ind2sub(size(gamma_grid), best_idx);
best = struct();
best.alpha = alpha_grid(best_alpha_idx);
best.r = r_grid(best_r_idx);
best.gamma = best_gamma;
best.Kval = K_grid{best_alpha_idx, best_r_idx};
best.Psol = Psol_grid{best_alpha_idx, best_r_idx};
best.Zsol = Zsol_grid{best_alpha_idx, best_r_idx};
best.feasratio = feasratio_grid(best_alpha_idx, best_r_idx);

Kval = best.Kval;
fprintf('\nBest feasible controller: alpha = %g, r = %d, gamma = %.6g\n', ...
    best.alpha, best.r, best.gamma);
