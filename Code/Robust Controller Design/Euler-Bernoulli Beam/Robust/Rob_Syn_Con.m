...
z1 = pde_var('out',1,s);                w = pde_var('in');
z2 = pde_var('out');
y = pde_var('sense');              u = pde_var('control');
dev = 0.7;
% Declare the sytem equations
PDE = [diff(x1,t) == -1*diff(x2,s,2) + wd1 +s^2*u+s^2*w;
...
data.vars = PIE.vars;
data.T = PIE.T;
data.A = PIE.A;           data.Bd = PIE.B1(:,2);  data.Bw = PIE.B1(:,1); data.Bu = PIE.B2;
data.Cd = PIE.C1(2,:); data.Dd = PIE.D11(2,2); data.Ddw = PIE.D11(2,1); data.Ddu = PIE.D12(2,1);
data.Cz = PIE.C1(1,:);  data.Dzd = PIE.D11(1,2); data.Dzw = PIE.D11(1,1); data.Dzu = PIE.D12(1,1);
...
%% 
% Declare the sytem equations
PDE = [diff(x1,t) == -1*diff(x2,s,2) - dev*diff(x2,s,2) +s^2*u+s^2*w;
    diff(x2,t) == diff(x1, s, 2);
    % z1 == u;
    z2 == int((1-s)*x2,s,[a,b]);
    subs(x1,s,a)==0;
    subs(diff(x1,s,1),s,a) == 0;
    subs(x2,s,b) == 0;
    subs(diff(x2,s,1),s,b)==0];           % second boundary condition
...
% opts.dt = 1;     % use time step of 10^-2
