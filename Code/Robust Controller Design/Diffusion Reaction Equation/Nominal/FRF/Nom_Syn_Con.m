...
omega = logspace(-1,4, 10);
invSettings = lpisettings("veryheavy");
opts.verbose = true;
FRF = opvarFreqresp(PIE_CL, omega, invSettings, opts);

%% 

figure
subplot(2,1,1)
semilogx(omega, mag2db(FRF.mag), 'LineWidth', 1.5); hold on
grid on
yline(mag2db(gam_val), "LineWidth", 1.5);
xlabel('\omega (rad/s)')
ylabel('Magnitude (dB)')
title('FRF using solve Ax = b')
ylim([-0.5,0.1])

subplot(2,1,2)
semilogx(omega, FRF.phase, 'LineWidth', 1.5)
grid on
xlabel('\omega (rad/s)')
ylabel('Phase (deg)')
...
figure
% subplot(3,1,1)
loglog(omega, sqrt(max(FRF.eta,0)), 'LineWidth', 1.5)
grid on
xlabel('\omega (rad/s)')
ylabel('\surd\gamma')
title('Residual bound from solve()')
